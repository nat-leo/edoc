...
type ActivityRequest = {
  ...
  from: string;
  to: string;
  metrics: Array<"submissions" | "logins">;
  groupBy: "day";
};
...
const MOCK_ACTIVITY_REQUEST: ActivityRequest = {
  userUuid: "mock-user-123",
  from: "2025-03-15",
  to: "2026-03-14",
...
const MOCK_ACTIVITY_RESPONSE: ActivityResponse = {
  request: MOCK_ACTIVITY_REQUEST,
  days: [
...
async function mockFetchActivity(request: ActivityRequest): Promise<ActivityResponse> {
...
export default function ContributionActivityMonitor() {
  ...
  React.useEffect(() => {
    ...
    async function loadActivity() {
      try {
        setIsLoading(true);
        const response = await mockFetchActivity(MOCK_ACTIVITY_REQUEST);
        if (!active) return;
  ...
  const dailyCounts = React.useMemo(
    () => buildDailyCounts(activity?.days ?? []),
    [activity]
  );
  ...
  return (
    <Card className="w-full rounded-2xl border border-zinc-300 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <CardContent className="px-6 pb-6">
        <div className="rounded-2xl border border-zinc-300 dark:border-zinc-700">
